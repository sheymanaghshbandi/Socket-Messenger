...
class ChatWindow(QWidget):
    ...
    def _message_bubble_max_width(self):
        ...
        return max(240, min(620, int(viewport_width * 0.68)))

    def _refresh_message_bubble_layouts(self):
        if not hasattr(self, "message_widgets"):
            return

        max_width = self._message_bubble_max_width()
        content_width = max(180, max_width - 34)

        for bubble in self.message_widgets.values():
            if bubble is None:
                continue
            ...
            if hasattr(bubble, "msg_label") and bubble.msg_label is not None:
                bubble.msg_label.setMaximumWidth(content_width)
                bubble.msg_label.updateGeometry()

            if hasattr(bubble, "caption_label") and bubble.caption_label is not None:
                bubble.caption_label.setMaximumWidth(content_width)
                bubble.caption_label.updateGeometry()

            if hasattr(bubble, "image_label") and bubble.image_label is not None:
                bubble.image_label.setMaximumWidth(min(content_width, 360))
                if hasattr(bubble, "_image_pixmap") and bubble._image_pixmap is not None and not bubble._image_pixmap.isNull():
                    bubble.image_label.setPixmap(
                        bubble._image_pixmap.scaled(
                            min(content_width, 360),
                            260,
                            Qt.KeepAspectRatio,
        ...
        if hasattr(self, "chat_area_widget"):
            self.chat_area_widget.adjustSize()

    def resizeEvent(self, event):
        super().resizeEvent(event)
        self._refresh_message_bubble_layouts()
    ...
    def draw_bubble(self, record):
        ...
        stack_widget.setSizePolicy(QSizePolicy.Maximum, QSizePolicy.Maximum)
        stack = QVBoxLayout(stack_widget)
        stack.setContentsMargins(0, 0, 0, 0)
        stack.setSpacing(4)
        stack.setSizeConstraint(QVBoxLayout.SetFixedSize)
        ...
        name_lbl = QLabel(display_name)
        name_color = "#F8EFFF" if (is_me and self.theme == "dark") else ("#6A35F0" if is_me else ("#D8E4FF" if self.theme == "dark" else "#1F2937"))
        name_lbl.setStyleSheet(f"""
            color: {name_color};
            font-size: 14px;
            font-weight: bold;
        ...
        bubble_lay.setSizeConstraint(QVBoxLayout.SetFixedSize)
        ...
        if message_type == "image":
            ...
            image_label.setMaximumSize(min(content_width, 360), 260)
            ...
            bubble.image_label = image_label
            bubble._image_pixmap = pixmap

            if not pixmap.isNull():
                thumb = pixmap.scaled(min(content_width, 360), 260, Qt.KeepAspectRatio, Qt.SmoothTransformation)
            ...
            bubble.caption_label = None
            if caption:
                caption_lbl = QLabel()
                caption_lbl.setWordWrap(True)
                caption_lbl.setTextFormat(Qt.PlainText)
                caption_lbl.setMaximumWidth(content_width)
            ...
            bubble.caption_label = None
            bubble.image_label = None
            bubble._image_pixmap = None
            bubble_lay.addWidget(edited_lbl)
        ...
        if is_me:
            row_lay.addStretch()
            row_lay.addWidget(stack_widget, 0, Qt.AlignRight)
            row_lay.addWidget(avatar, alignment=Qt.AlignTop)
        else:
            row_lay.addWidget(avatar, alignment=Qt.AlignTop)
            row_lay.addWidget(stack_widget, 0, Qt.AlignLeft)
            row_lay.addStretch()
